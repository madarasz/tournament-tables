...
class AllocationPerformanceTest extends TestCase
{
    ...
    /**
     * SC-002: Allocation generation must complete in < 10 seconds for 40 players.
     *
     * @group performance
     */
    public function testAllocationGenerationUnderTenSeconds(): void
    {
        // Create prior round allocations to build history
        $this->createPriorRoundHistory();

        // Create pairings for the final round
        ...
        $this->assertLessThan(
            ...
            $duration,
            sprintf(
                'Allocation generation took %.2f seconds, exceeding limit of %d seconds',
                $duration,
                self::MAX_ALLOCATION_TIME_SECONDS
            )
        );
